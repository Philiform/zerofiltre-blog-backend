...
public class CompanyController {
    ...
    public CompanyController(UserProvider userProvider, SecurityContextManager securityContextManager, CompanyProvider companyProvider, MessageSource sources) {
        this.securityContextManager = new SecurityContextManager(userProvider);
        this.companyService = new CompanyService(companyProvider, userProvider);
    ...
    public ResponseEntity<Company> getById(@PathVariable("id") long companyId) throws ResourceNotFoundException, ForbiddenActionException {
        User user = securityContextManager.getAuthenticatedUser();
        Optional<Company> company = companyService.getById(user, companyId);
